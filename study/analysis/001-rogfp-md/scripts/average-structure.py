...
def main():
    ...
    data_dir = os.path.join(base_dir, "analysis/001-rogfp-md/data/structure")
    os.makedirs(data_dir, exist_ok=True)

    u = mda.Universe(topology_path, trajectory_path)
    data_path = os.path.join(data_dir, "average-structure-cro.pdb")
    align.AverageStructure(
        u, filename=data_path, select="resname CRO", ref_frame=0
    ).run()
