...
if __name__ == "__main__":
    ...
    rogfp_data_path = os.path.join(
        base_dir, "analysis/001-rogfp-md/data/struct-desc/cro65_oh-cym202_ca-dist.npy"
    )
    ...
    rogfp_oxd_data_path = os.path.join(
        base_dir,
        "analysis/004-rogfp-oxd-md/data/struct-desc/cro65_oh-cyx202_ca-dist.npy",
    )
    rogfp_oxd_data = np.load(rogfp_oxd_data_path)

    rogfp_cu_data_path = os.path.join(
        base_dir,
        "analysis/003-rogfp-cu-md/data/struct-desc/cro65_oh-cym202_ca-dist.npy",
    )
    ...
    # Make pdf plot
    fig_title = "b007-cro66_oh-cys204_ca"
    pdf_plt_kwargs = {"alpha": 1.0, "linewidth": 2.5}
    x_label = "CRO66 OH - CYS204 CA Distance [Å]"
    plot_x_bounds = (4.5, 9)
    y_label = "Density"
    ...
    pmf_rogfp, pmf_rogfp_oxd, pmf_rogfp_cu = compute_pmfs(
        x_values, 8.04, (pdf_rogfp, pdf_rogfp_oxd, pdf_rogfp_cu), T=300.0
    )
