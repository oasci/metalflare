...
if __name__ == "__main__":
    ...
    rogfp_data_path = os.path.join(
        base_dir,
        "analysis/005-rogfp-glh-md/data/struct-desc/cys202_o-phe221_h-dist.npy",
    )
    rogfp_data = np.load(rogfp_data_path)
    # Oxidized
    rogfp_oxd_data_path = os.path.join(
        base_dir,
        "analysis/007-rogfp-oxd-glh-md/data/struct-desc/cys202_o-phe221_h-dist.npy",
    )
    rogfp_oxd_data = np.load(rogfp_oxd_data_path)
    # Copper
    rogfp_cu_data_path = os.path.join(
        base_dir,
        "analysis/006-rogfp-cu-glh-md/data/struct-desc/cys202_o-phe221_h-dist.npy",
    )
    ...
    # Make pdf plot
    fig_title = "f012-cys204_o-phe223_h"
    pdf_plt_kwargs = {"alpha": 1.0, "linewidth": 2.5}
    x_label = "Cys204 O - Phe223 H Distance [Å]"
    plot_x_bounds = (1, 3)
    y_label = "Density"
    ...
    pmf_rogfp, pmf_rogfp_oxd, pmf_rogfp_cu = compute_pmfs(
        x_values, 1.87, (pdf_rogfp, pdf_rogfp_oxd, pdf_rogfp_cu), T=300.0
    )
