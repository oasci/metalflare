...
if __name__ == "__main__":
    ...
    rogfp_data_path = os.path.join(
        base_dir, "analysis/001-rogfp-md/data/struct-desc/cym145_sg-cym202_sg-dist.npy"
    )
    rogfp_data = np.load(rogfp_data_path)
    # Oxidized
    rogfp_oxd_data_path = os.path.join(
        base_dir,
        "analysis/004-rogfp-oxd-md/data/struct-desc/cyx145_sg-cyx202_sg-dist.npy",
    )
    rogfp_oxd_data = np.load(rogfp_oxd_data_path)
    # Copper
    rogfp_cu_data_path = os.path.join(
        base_dir,
        "analysis/003-rogfp-cu-md/data/struct-desc/cym145_sg-cym202_sg-dist.npy",
    )
    ...
    # Make pdf plot
    fig_title = "f009-cys147_sg-cys204_ca"
    pdf_plt_kwargs = {"alpha": 0.5, "linewidth": 1.0}
    x_label = "CYS147 CA - CYS204 SG Distance [Å]"
    plot_x_bounds = (1, 6)
    y_label = "Density"
