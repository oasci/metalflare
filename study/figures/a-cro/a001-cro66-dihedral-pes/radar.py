...
def load_and_process_data(base_dir, data_str, state):
    path = os.path.join(base_dir, f"analysis/{names_state[state]}/data/struct-desc/{data_str}.npy")
    data = np.load(path)
    data = np.degrees(data)  # Convert to degrees
    data = np.concatenate([data, data + 360, data - 360])  # Extend the data for circular KDE
    return data
...
if __name__ == "__main__":
    ...
    colors = {
        'reduced': '#1e2e79', 'oxidized': '#EC4067', 'cu': '#f99752', "na": "#1b998b"
    }
    ...
    for dtype in data_types:
        ...
        ax.plot(angles, kde_values_x, label=f"{dtype.capitalize()} - {data_x_label}",
                color=colors[dtype], linewidth=1.5)
        ax.plot(angles, kde_values_y, label=f"{dtype.capitalize()} - {data_y_label}",
                color=colors[dtype], linewidth=1.5, linestyle='--')
