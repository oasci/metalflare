...
fig_label = "pes"
data_x_str = "his146_n_ca_c-asn147_n-dihedral"
data_x_label = "His148 $\psi$ [deg.]"
...
if __name__ == "__main__":
    ...
    fig = create_pes(
        data_x_red, data_y_red, bins=n_bins, vmin=0, vmax=3.2, levels=15, T=300.0
    )
    ...
    fig = create_pes(
        data_x_oxd, data_y_oxd, bins=n_bins, vmin=0, vmax=3.2, levels=15, T=300.0
    )
    plt.xlabel(data_x_label)
    plt.xlim(*x_lims)
    plt.xticks(x_ticks)
    ...
    fig = create_pes(
        data_x_cu, data_y_cu, bins=n_bins, vmin=0, vmax=3.2, levels=15, T=300.0
    )
    plt.xlabel(data_x_label)
    plt.xlim(*x_lims)
    plt.xticks(x_ticks)
